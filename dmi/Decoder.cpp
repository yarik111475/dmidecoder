...
//part for windows
    ...
    for(auto iterator=content_.begin();iterator<content_.end();++iterator){
        ...
        const int& handle {static_cast<unsigned char>(header_buffer.at(2)) * 0x100 +
                           static_cast<unsigned char>(header_buffer.at(3))};
...
QJsonObject Decoder::decode_structure(const Structure &dmi, int type)
{
    QJsonObject out_object {};
    switch(type){
    case 0:
        out_object=bios_information(dmi);
        break;
    case 1:
        out_object=system_information(dmi);
        break;
    case 2:
        out_object=baseboard_information(dmi);
        break;
    case 3:
        out_object=chassis_information(dmi);
        break;
    case 4:
        out_object=processor_information(dmi);
        break;
    case 5:
        out_object=memory_controller_information(dmi);
        break;
    case 6:
        out_object=memory_module_information(dmi);
        break;
    case 7:
        out_object=cache_information(dmi);
        break;
    case 8:
        out_object=port_connector_information(dmi);
        break;
    case 9:
        out_object=system_slot_information(dmi);
        break;
    case 10:
        out_object=onboard_device_information(dmi);
        break;
    case 11:
        out_object=oem_strings(dmi);
        break;
    case 12:
        out_object=system_configuration_options(dmi);
        break;
    case 13:
        out_object=bios_language_information(dmi);
        break;
    case 16:
        out_object=physical_memory_array(dmi);
        break;
    case 17:
        out_object=memory_device(dmi);
        break;
    case 18:
        out_object=memory_error_information(dmi);
        break;
    case 21:
        out_object=builtin_pointing_device(dmi);
        break;
    case 22:
        out_object=portable_battery(dmi);
        break;
    case 26:
        out_object=voltage_probe(dmi);
        break;
    case 27:
        out_object=cooling_device(dmi);
        break;
    case 28:
        out_object=temperature_probe(dmi);
        break;
    case 29:
        out_object=electrical_current_probe(dmi);
        break;
    case 34:
        out_object=management_device_information(dmi);
        break;
    case 37:
        break;
    case 38:
    ...
    case 40:
        break;
    case 41:
        out_object=onboard_device_extended_information(dmi);
        break;
    case 42:
        break;
    case 44:
        out_object=processor_additional_information(dmi);
        break;
    }
    if(!out_object.isEmpty()){
        //out_object.insert("hash", dmi.hash_);
        out_object.insert("type", dmi.type_);
        out_object.insert("handle", dmi.handle_);
    }

    return out_object;
}

std::vector<std::pair<QString, QJsonObject> > Decoder::decode_information()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
    ...
    if(ep_success){
        ...
        if(!structure_list_.empty()){
            ...
            for(const Structure& dmi: structure_list_){
                const QJsonObject dmi_object {decode_structure(dmi, dmi.type_)};
                if(!dmi_object.isEmpty()){
                    dmi_list_.push_back(std::make_pair(dmi_object.value("object_type").toString(),
                                      dmi_object));
                }
            }
...
//Type 0
QJsonObject Decoder::bios_information(const Structure &dmi)
{
    ...
    if((dmi.strings_.size() > vendor_locator) && (vendor_locator >= 0)){
        vendor=QString::fromStdString(dmi.strings_.at(vendor_locator)).simplified();
    }
    ...
    if((dmi.strings_.size() > version_locator) && (version_locator >= 0)){
        version=QString::fromStdString(dmi.strings_.at(version_locator)).simplified();
    }
    ...
    if((dmi.strings_.size() > release_locator) && (release_locator >= 0)){
        release_date=QString::fromStdString(dmi.strings_.at(release_locator)).simplified();
    }
    ...
    if(dmi.data_.size() > 0x15){
        ...
        bios_release=QString("%1.%2").arg(major_release,minor_release);
    }
...
//Type 1
QJsonObject Decoder::system_information(const Structure &dmi)
{
    ...
    QString manufacturer("Unknoun");
    if((manufacturer_locator >= 0) && (dmi.strings_.size() > manufacturer_locator)){
        manufacturer=QString::fromStdString(dmi.strings_.at(manufacturer_locator)).simplified();
    }
    ...
    if((product_locator >= 0) && (dmi.strings_.size() > product_locator)){
        product_name=QString::fromStdString(dmi.strings_.at(product_locator)).simplified();
    }
    ...
    QString serial_number {"Unknown"};
    if((serial_locator>=0) && (dmi.strings_.size()>serial_locator)){
        serial_number=QString::fromStdString(dmi.strings_.at(serial_locator)).simplified();
    }

    QString uuid {"Unknown"};
    if(dmi.data_.size() > 0x18){
        std::vector<char> uuid_data;
        uuid_data.assign(dmi.data_.begin()+0x08,dmi.data_.begin()+0x18);

        const QByteArray q_uuid_data (uuid_data.data(),uuid_data.size());
        const QUuid& q_uuid {QUuid::fromRfc4122(q_uuid_data)};
        uuid=q_uuid.toString(QUuid::WithoutBraces);
    }

    const QString& wakeup_type {dmi.data_.size() > 0x18 ?
                   wakeup_get(static_cast<unsigned char>(dmi.data_.at(0x18))) : "Unknown"};

    const int& sku_locator {(dmi.data_.size() > 0x19) ?
                    (static_cast<unsigned char>(dmi.data_.at(0x19)-1)) : -1};

    QString sku_number {"Unknown"};
    if((sku_locator>=0) && (dmi.strings_.size()>sku_locator)){
        sku_number=QString::fromStdString(dmi.strings_.at(sku_locator)).simplified();
    }
    ...
    if((family_locator>=0) && (dmi.strings_.size()>family_locator)){
        family=QString::fromStdString(dmi.strings_.at(family_locator)).simplified();
    }
...
//Type 2
QJsonObject Decoder::baseboard_information(const Structure &dmi)
{
    ...
    if((product_locator>=0) && (dmi.strings_.size()>product_locator)){
        product=QString::fromStdString(dmi.strings_.at(product_locator)).simplified();
    }
    ...
    QString asset_tag {"Unknown"};
    if((asset_locator>=0) && (dmi.strings_.size()>asset_locator)){
        asset_tag=QString::fromStdString(dmi.strings_.at(asset_locator)).simplified();
    }

    const QStringList& feature {dmi.data_.size() > 0x09 ?
                    feature_get(static_cast<unsigned char>(dmi.data_.at(0x09))) : QStringList{}};

    const int& chassis_locator {(dmi.data_.size() > 0x0A) ?
                    (static_cast<unsigned char>(dmi.data_.at(0x0A)-1)) : -1};

    QString chassis_location {"Unknown"};
    if((chassis_locator>=0) && (dmi.strings_.size()>chassis_locator)){
        chassis_location=QString::fromStdString(dmi.strings_.at(chassis_locator)).simplified();
    }

    const QString& board_type {dmi.data_.size() > 0x0D ?
                    board_get(static_cast<unsigned char>(dmi.data_.at(0x0D))) : "Unknown"};
...
//Type 3
QJsonObject Decoder::chassis_information(const Structure &dmi)
{
    //get chassis type
    ...
    const QString& manufacturer (manufacturer_locator>=0 && dmi.strings_.size()>manufacturer_locator ?
                                     QString::fromStdString(dmi.strings_.at(manufacturer_locator)).simplified() : "Unknown");

    const QString& type(dmi.data_.size()>0x05 ?
                            type_get(static_cast<unsigned char>(dmi.data_.at(0x05))) : "Unknown");

    const int& version_locator (dmi.data_.size()>0x06 ?
                                         static_cast<unsigned char>(dmi.data_.at(0x06))-1 : -1);
    const QString& version (version_locator>=0 && dmi.strings_.size()>version_locator ?
                                     QString::fromStdString(dmi.strings_.at(version_locator)).simplified() : "Unknown");

    const int& serial_locator (dmi.data_.size()>0x07 ?
                                         static_cast<unsigned char>(dmi.data_.at(0x07))-1 : -1);
    const QString& serial_number (serial_locator>=0 && dmi.strings_.size()>serial_locator ?
                                     QString::fromStdString(dmi.strings_.at(serial_locator)).simplified() : "Unknown");

    const int& asset_locator (dmi.data_.size()>0x08 ?
                                         static_cast<unsigned char>(dmi.data_.at(0x08))-1 : -1);
    const QString& asset_tag (asset_locator>=0 && dmi.strings_.size()>asset_locator ?
                                     QString::fromStdString(dmi.strings_.at(asset_locator)).simplified() : "Unknown");

    const QString& bootup_state (dmi.data_.size()>0x09 ?
                                     chassis_state_get(static_cast<unsigned char>(dmi.data_.at(0x09))) : "Unknown");

    const QString& power_supply_state (dmi.data_.size()>0x0A ?
                                     chassis_state_get(static_cast<unsigned char>(dmi.data_.at(0x0A))) : "Unknown");

    const QString& thermal_state (dmi.data_.size()>0x0B ?
                                     chassis_state_get(static_cast<unsigned char>(dmi.data_.at(0x0B))) : "Unknown");

    const QString& security_status (dmi.data_.size()>0x0C ?
                                        security_status_get(static_cast<unsigned char>(dmi.data_.at(0x0C))) : "Unknown");
    ...
    const QString& sku_number (sku_locator>=0 && dmi.strings_.size()>sku_locator ?
                                   QString::fromStdString(dmi.strings_.at(sku_locator)).simplified() :
                                   "Unknown");
...
//Type 4
QJsonObject Decoder::processor_information(const Structure &dmi)
{
    //get processor type
    const auto& type_get{[](unsigned char key){
            const std::map<unsigned char,QString>& type_map{
    ...
    const auto& voltage_get{[](unsigned char key){
            ...
            const std::map<unsigned char,QString>& voltage_map{
                {0b001,"5v"},{0b010,"3.3v"},{0b100,"2.9v"}
            };
            if(!(key & 0x80)){
    ...
    const auto& populated_get{[](unsigned char key){
            const std::map<unsigned char,QString>& p_map{
                {0x00,"CPU Socket Unpopulated"},
                {0x41,"CPU Socket Populated"}
            };
            QStringList out;
    ...
    QString socket_designation {"Unknown"};
    if((socket_locator >=0) && dmi.strings_.size()>socket_locator){
        socket_designation=QString::fromStdString(dmi.strings_.at(socket_locator)).simplified();
    }
    ...
    QString processor_manufacturer {"Unknown"};
    if((processor_manufacturer_locator >=0) && dmi.strings_.size()>processor_manufacturer_locator){
        processor_manufacturer=QString::fromStdString(dmi.strings_.at(processor_manufacturer_locator)).simplified();
    }
    ...
    if(dmi.data_.size()>0x0A){
        ...
        QByteArray q_id_data (id_data.data(),id_data.size());
        processor_id=q_id_data.toHex();
    }

    const int& version_locator {dmi.data_.size() > 0x10 ?
                     (static_cast<unsigned char>(dmi.data_.at(0x10))-1) : -1};

    QString processor_version {"Unknown"};
    if((version_locator >=0) && dmi.strings_.size()>version_locator){
        processor_version=QString::fromStdString(dmi.strings_.at(version_locator)).simplified();
    }

    const QStringList& voltage (dmi.data_.size()>0x11 ?
                                    voltage_get(static_cast<unsigned char>(dmi.data_.at(0x11))) : QStringList{});
    ...
    const QStringList& populated_status {populated_get(status_key)};
    ...
    QString part_number {"Unknown"};
    if((part_locator >=0) && dmi.strings_.size()>part_locator){
        part_number=QString::fromStdString(dmi.strings_.at(part_locator)).simplified();
    }
    ...
    const QStringList& processor_chracteristics(dmi.data_.size()>0x26 ?
    ...
    const QJsonObject& out_object{
        ...
        {"status", status},
        {"populated_status", populated_status.join(", ")},
        ...
        {"processor_chracteristics", processor_chracteristics.join(",")},
...
//Type 5, Obsolete
QJsonObject Decoder::memory_controller_information(const Structure &dmi)
{
    //get error detecting
...
//Type 6, Obsolete
QJsonObject Decoder::memory_module_information(const Structure &dmi)
{
    //get memory type
    ...
    const QString& socket_designation {(designation_locator>=0) && (dmi.strings_.size()>designation_locator) ?
                    QString::fromStdString(dmi.strings_.at(designation_locator)).simplified() :
                    "Unknown"};

    const int& bank_connections {dmi.data_.size()>0x05 ?
                    static_cast<unsigned char>(dmi.data_.at(0x05)) : 0};

    const int& current_speed {dmi.data_.size()>0x05 ?
                    static_cast<unsigned char>(dmi.data_.at(0x05)) : 0};

    const int& current_memory_type_key {dmi.data_.size()>0x00 ?
                static_cast<unsigned char>(dmi.data_.at(0x00)) * 0x100 +
                static_cast<unsigned char>(dmi.data_.at(0x00)) : 0};

    const QString& current_memory_type {type_get(current_memory_type_key)};
    ...
    const QJsonObject& out_object {
        ...
        {"current_speed", QString("%1 ns").arg(current_speed)},
...
//Type 7
QJsonObject Decoder::cache_information(const Structure &dmi)
{
    ...
    const auto& configuration_get{[&location_get, &mode_get](unsigned short key){
            ...
            const int& level {static_cast<unsigned char>(key>>13)};
            const QString& level_str {QString("Level: %1").
                                      arg(QString::number(level))};
            const QString& socketed {(static_cast<unsigned char>(key & 0x08))!=0 ?
                            "Socketed" : "Not Socketed"};
            ...
            out<<level_str<<socketed<<location<<enabled<<operation_mode;
            return out;
        }
    };
     ...
     const int& handle {dmi.data_.size()>0x03 ?
                     (static_cast<unsigned char>(dmi.data_.at(0x03)) * 0x100 +
                      static_cast<unsigned char>(dmi.data_.at(0x02))) : 0};

     const int& designation_locator {dmi.data_.size()>0x04 ?
                     static_cast<unsigned char>(dmi.data_.at(0x04))-1 : -1};

     const QString& socket_designation {dmi.strings_.size()>designation_locator && designation_locator>=0 ?
                     QString::fromStdString(dmi.strings_.at(designation_locator)).simplified() : "Unknown"};
     ...
     const QString& cache_speed {QString("%1 ns").arg(dmi.data_.size()>0x0F ?
...
//Type 8
QJsonObject Decoder::port_connector_information(const Structure &dmi)
{
    ...
    const QString& internal_reference_designator ((internal_reference_locator>=0) && (dmi.strings_.size()>internal_reference_locator) ?
                QString::fromStdString(dmi.strings_.at(internal_reference_locator)).simplified() :
                                                      "Unknown");
    ...
    const QString& external_reference_designator ((external_reference_locator>=0) && (dmi.strings_.size()>external_reference_locator) ?
                QString::fromStdString(dmi.strings_.at(external_reference_locator)).simplified() :
                                                      "Unknown");
...
//Type 9
QJsonObject Decoder::system_slot_information(const Structure &dmi)
{
    //get slot type
    ...
    const int& slot_designation_locator (dmi.data_.size()>0x04 ?
                                             static_cast<unsigned char>(dmi.data_.at(0x04))-1 : -1);
    const QString& slot_designation (dmi.strings_.size()>slot_designation_locator &&
                                     slot_designation_locator>=0 ? QString::fromStdString(dmi.strings_.at(slot_designation_locator)).simplified() :
                                                                   "Unknown");
    ...
    const QString& slot_physical_width {dmi.data_.size()>0x14 ?
                    physical_width_get(static_cast<unsigned char>(dmi.data_.at(0x14))) : "Unknown"};
...
//Type 10 Obsolete
QJsonObject Decoder::onboard_device_information(const Structure &dmi)
{
    const auto& type_get{[](unsigned char key){
            const std::map<unsigned char,QString>& type_map{
                {0x01,"Other"},
...
//Type 11
QJsonObject Decoder::oem_strings(const Structure &dmi)
{
    QJsonArray out_array;
    for(const std::string& string: dmi.strings_){
        out_array.append(QString::fromStdString(string).simplified());
    }
    return QJsonObject {
        //{"object_id", dmi.type_},
        {"object_type", "oem_strings"},
        {"oem_strings",out_array}
    };
}

//Type 12
QJsonObject Decoder::system_configuration_options(const Structure &dmi)
{
    QJsonArray out_array;
    for(const std::string& string: dmi.strings_){
        out_array.append(QString::fromStdString(string).simplified());
    ...
    const QJsonObject& out_object {
        ...
        {"system_configuration_options", out_array}
    };
    return out_object;
}

//Type 13
QJsonObject Decoder::bios_language_information(const Structure &dmi)
{
    QJsonArray out_array;
    for(const std::string& string: dmi.strings_){
        out_array.append(QString::fromStdString(string).simplified());
    ...
    const QJsonObject& out_object {
        ...
        {"installable_languages", out_array}
    };
    return out_object;
}
...
void Decoder::group_associations(const Structure &dmi)
{
    ...
    for(int i=begin_;i<dmi.data_.size(); i+=group_size){
        ...
        const QString& group_name {group_name_locator>=0 && dmi.strings_.size()>group_name_locator ?
                        QString::fromStdString(dmi.strings_.at(group_name_locator)).simplified() :
                        "Unknown"};
        ...
        std::for_each(structure_list_.begin(),structure_list_.end(),[&item_type, &item_handle,this](const Structure& dmi){
            if(dmi.type_==item_type){
                const QJsonObject& dmi_obj {decode_structure(dmi,item_handle)};
                if(!dmi_obj.isEmpty()){
                    dmi_list_.push_back(std::make_pair(dmi_obj.value("object_type").toString(),dmi_obj));
                }
            }
        });
    }
...
//Type 16
QJsonObject Decoder::physical_memory_array(const Structure &dmi)
{
    //get location
...
//Type 17
QJsonObject Decoder::memory_device(const Structure &dmi)
{
    //get memory form-factor
    const auto& form_factor_get{[](uchar key){
            const std::map<uchar,QString>& factor_map{
                {0x01,"Other"},
                {0x02,"Unknown"},
                {0x03,"SIMM"},
                {0x04,"SIP"},
    ...
    //get memory type
    const auto& type_get{[](uchar key){
            const std::map<uchar,QString> type_map {
                {0x01,"Other"},
                {0x02,"Unknown"},
                {0x03,"DRAM"},
                {0x04,"EDRAM"},
    ...
    const auto& type_detail_get{[](unsigned short key){
            const std::map<uchar,QString> detail_map {
                {0x00,"Reserved"},
                {0x01,"Other"},
                {0x02,"Unknown"},
                {0x04,"Fast-paged"},
    ...
    const auto& technology_get{[](uchar key){
            const std::map<uchar,QString> technology_map{
                {0x01,"Other"},
                {0x02,"Unknown"},
                {0x03,"DRAM"},
                {0x04,"NVDIMM-N"},
    ...
    if((dmi.strings_.size() > device_locator) && (device_locator >= 0)){
        device=QString::fromStdString(dmi.strings_.at(device_locator)).simplified();
    }

    QString bank {"Unknown"};
    ...
    if((dmi.strings_.size() > bank_locator) && (bank_locator >= 0)){
        bank=QString::fromStdString(dmi.strings_.at(bank_locator)).simplified();
    }

    const QString& memory_type {dmi.data_.size() > 0x12 ?
                   type_get(static_cast<unsigned char>(dmi.data_.at(0x12))) : "Unknown"};

    const QStringList& type_detail {dmi.data_.size() > 0x14 ?
                   type_detail_get((static_cast<unsigned char>(dmi.data_.at(0x14))) * 0x100 +
                                   static_cast<unsigned char>(dmi.data_.at(0x13))) : QStringList{}};
    ...
    const QString& memory_technology {dmi.data_.size() > 0x28 ?
                    technology_get(static_cast<unsigned char>(dmi.data_.at(0x28))) : "Unknown"};
    ...
    QString firmware_version {"Unknown"};
    ...
    if((dmi.strings_.size() > fw_version_locator) && (fw_version_locator >= 0)){
        firmware_version=QString::fromStdString(dmi.strings_.at(fw_version_locator));
    }
    ...
    const QJsonObject& out_object {
        ...
        {"size", QString::number(real_size).toLongLong()},
...
//Type 18
QJsonObject Decoder::memory_error_information(const Structure &dmi)
{
    //get error type
...
//Type 21
QJsonObject Decoder::builtin_pointing_device(const Structure &dmi)
{
    //get type
    const auto& type_get{[](unsigned char key){
            const std::map<unsigned char,QString>& type_map{
    ...
    const QString& type(dmi.data_.size()>0x04 ?
                            type_get(static_cast<unsigned char>(dmi.data_.at(0x04))) :
                            "Unknown");
...
//Type 22
QJsonObject Decoder::portable_battery(const Structure &dmi)
{
   ...
   if((dmi.strings_.size()>location_locator) && (location_locator>=0)){
       location=QString::fromStdString(dmi.strings_.at(location_locator)).simplified();
   }
   ...
   if((dmi.strings_.size()>date_locator) && (date_locator>=0)){
       manufacture_date=QString::fromStdString(dmi.strings_.at(date_locator)).simplified();
   }
   ...
   if((dmi.strings_.size()>name_locator) && (name_locator>=0)){
       device_name=QString::fromStdString(dmi.strings_.at(name_locator)).simplified();
   }

   const QString& device_chemistry (dmi.data_.size()>0x09 ?
                                        chemistry_get(static_cast<unsigned char>(dmi.data_.at(0x09))) : "Unknown");

   const int sdbs_chemistry_locator (dmi.data_.size()>0x14 ?
                                         static_cast<unsigned char>(dmi.data_.at(0x14))-1 : -1);
   const QString& sdbs_device_chemistry (dmi.strings_.size()>sdbs_chemistry_locator &&
                                  sdbs_chemistry_locator>=0 ?
                                      QString::fromStdString(dmi.strings_.at(sdbs_chemistry_locator)).simplified() : "Unknown");
...
//Type 26
QJsonObject Decoder::voltage_probe(const Structure &dmi)
{
    ...
    const QString& description {description_locator>=0 && dmi.strings_.size()>description_locator ?
                    QString::fromStdString(dmi.strings_.at(description_locator)).simplified() :
                    "Unknown"};
...
//Type 27
QJsonObject Decoder::cooling_device(const Structure &dmi)
{
    //get device type
    ...
    const QString& device_type {dmi.data_.size()>0x06 ?
                    device_type_get(static_cast<unsigned char>(dmi.data_.at(0x06))).simplified() :
                    "Unknown"};

    const QString& device_status {dmi.data_.size()>0x06 ?
                    device_status_get(static_cast<unsigned char>(dmi.data_.at(0x06))).simplified() :
                    "Unknown"};
...
//Type 28
QJsonObject Decoder::temperature_probe(const Structure &dmi)
{
...
//Type 29
QJsonObject Decoder::electrical_current_probe(const Structure &dmi)
{
   QJsonObject out_object {voltage_probe(dmi)};
   out_object.insert("object_type","electrical_current_probe");
   return out_object;
}

//Type 34
QJsonObject Decoder::management_device_information(const Structure &dmi)
{
    //get device type
    const auto& type_get{[](unsigned char key){
            const std::map<unsigned char,QString> type_map{
    ...
    const QString& description {(description_locator>=0) && (dmi.strings_.size()>description_locator) ?
                QString::fromStdString(dmi.strings_.at(description_locator)).simplified() : "Unknown"};
...
//Type 41
QJsonObject Decoder::onboard_device_extended_information(const Structure &dmi)
{
    const auto& type_get{[](unsigned char key){
            const std::map<unsigned char,QString>& type_map{
                {0x01,"Other"},
    ...
    //get device status
    const auto& status_get{[](unsigned char key){
            return ((key & 0x80)>0 ? "Enabled" : "Disabled");
        }
    };
    ...
    const QString& reference_designation ((reference_locator>=0) && (dmi.strings_.size()>reference_locator) ?
                                              QString::fromStdString(dmi.strings_.at(reference_locator)).simplified() :
                                              "Unknown");

    const QString& device_type (dmi.data_.size()>0x05 ?
                                    type_get(static_cast<unsigned char>(dmi.data_.at(0x05))).simplified() :
                                    "Unknown");
...
//Type 44
QJsonObject Decoder::processor_additional_information(const Structure &dmi)
{
    return QJsonObject{
        //{"object_id", dmi.type_},
